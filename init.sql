...
CREATE TABLE IF NOT EXISTS events (
    ...
    type ENUM('Seminar', 'Workshop', 'Conference', 'Training', 'Webinar', 'Other') NOT NULL,
...
-- Multi-template for up to 6 certificate designs per event
CREATE TABLE IF NOT EXISTS certificate_templates_multi (
    id INT AUTO_INCREMENT PRIMARY KEY,
    event_id INT NOT NULL,
    template_index INT NOT NULL, -- 1 sampai 6
    template_path VARCHAR(255) NOT NULL,
    template_fields JSON,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (event_id) REFERENCES events(id) ON DELETE CASCADE,
    UNIQUE KEY unique_event_template (event_id, template_index)
);
...
INSERT IGNORE INTO participants (id, ticket_id, name, email, phone, address, registered_at) VALUES
(1, 3, 'John Smith', 'john.smith@email.com', '+62812345678', 'Jakarta, Indonesia', '2025-06-01 10:30:00'),
(2, 4, 'Sarah Johnson', 'sarah.johnson@email.com', '+62823456789', 'Surabaya, Indonesia', '2025-06-02 14:15:00'),
(3, 6, 'Michael Chen', 'michael.chen@email.com', '+62834567890', 'Bandung, Indonesia', '2025-06-03 09:45:00'),
(4, 8, 'Emily Davis', 'emily.davis@email.com', '+62845678901', 'Medan, Indonesia', '2025-06-04 16:20:00'),
(5, 11, 'David Wilson', 'david.wilson@email.com', '+62856789012', 'Semarang, Indonesia', '2025-06-05 11:10:00'),
(6, 14, 'Lisa Anderson', 'lisa.anderson@email.com', '+62867890123', 'Yogyakarta, Indonesia', '2025-06-06 13:30:00');
...
GROUP BY e.id, e.name, e.slug, e.type, e.location, e.start_time, e.end_time, e.quota, e.ticket_design;
