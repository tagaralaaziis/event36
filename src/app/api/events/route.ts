...
import db, { testConnection, logSystemEvent } from '@/lib/db'
...
export async function POST(request: NextRequest) {
  try {
    ...
    await logSystemEvent('event_create', `Event created: ${name}`, { eventId });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('📝 Updating event:', { name, slug, type, location, quota })
    ...
    // Check if slug already exists
    const [existingSlug] = await db.execute('SELECT id FROM events WHERE slug = ?', [slug])
    if ((existingSlug as any[]).length > 0 && (existingSlug as any[])[0].id !== eventId) {
      return NextResponse.json({ message: 'Slug already exists. Please use a different slug.' }, { status: 400 })
    }
    ...
    await db.execute(`
      UPDATE events SET name = ?, slug = ?, type = ?, location = ?, description = ?, start_time = ?, end_time = ?, quota = ?, ticket_design = ?, ticket_design_size = ?, ticket_design_type = ?
      WHERE id = ?
    ...
    await logSystemEvent('event_update', `Event updated: ${eventId}`, { eventId, updateData: { name, slug, type, location, description, startTime, endTime, quota, ticketDesignPath, ticketDesignSize, ticketDesignType } });
