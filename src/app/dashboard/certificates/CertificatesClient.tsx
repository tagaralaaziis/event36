...
export default function CertificatesClient({ certificates: initialCertificates, uniqueEvents }: { certificates: any[], uniqueEvents: any[] }) {
  const { data, error, isLoading, mutate } = useSWR('/api/certificates', fetcher, {
  ...
  const handleSend = async (id: string) => {
    setSendingId(id)
    try {
      const res = await fetch(`/api/certificates/send/${id}`, { method: 'POST' })
      if (res.ok) {
        toast.success('Sertifikat berhasil dikirim!')
        mutate()
      } else {
        const data = await res.json()
        toast.error('Gagal mengirim: ' + (data.message || 'Unknown error'))
      }
    } catch (err) {
  ...
  const handleBulkSend = async () => {
    setBulkSending(true)
    const pending = filteredCertificates.filter((c: any) => !c.sent)
    for (const cert of pending) {
      await handleSend(cert.id)
    }
    setBulkSending(false)
    toast.success('Semua sertifikat pending telah dikirim!')
  }
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <tr key={certificate.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          ...
                          <button
                            className={`text-green-600 hover:text-green-900 flex items-center space-x-1 ${sendingId === certificate.id ? 'opacity-50 cursor-wait' : ''}`}
                            disabled={certificate.sent || !!sendingId}
                            onClick={() => handleSend(certificate.id)}
                            title={certificate.sent ? 'Sudah terkirim' : 'Kirim ke email peserta'}
                          >
                            <Send className="h-4 w-4" />
                            <span>{sendingId === certificate.id ? 'Mengirim...' : 'Send'}</span>
                          </button>
                        </div>
                      </td>
                    </tr>
            ...
            <div className="px-6 py-4 border-t border-gray-200">
              <button
                ...
                disabled={bulkSending || filteredCertificates.filter((c: any) => !c.sent).length === 0}
              >
                <Send className="h-4 w-4" />
                <span>{bulkSending ? 'Mengirim...' : 'Kirim Semua Pending'}</span>
              </button>
            </div>
          </div>
        ) : (
